...
class contactForm{
	...
	function __construct($cfg){
		...
		$this->url = (isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
	...
	function sendAdminNotification(){
		...
		if($this->demo != 1)
		{
			...
			$this->phpmailer_adminnotification->SetFrom($param['reply_emailaddress']);
			...
			if($this->cfg['email_address_cc']){
				...
				$explode_cc = array_map('trim', explode(',', $this->cfg['email_address_cc']));

				foreach($explode_cc as $explode_cc_value){
			...
			if($this->cfg['email_address_bcc']){
				...
				$explode_bcc = array_map('trim', explode(',', $this->cfg['email_address_bcc']));

				foreach($explode_bcc as $explode_bcc_value){
			...
			$this->phpmailer_adminnotification->IsHTML(false);
				
			if($this->mail_content_type_format == 'html'){
				$this->phpmailer_adminnotification->IsHTML(true);
			}
			...
			if($this->cfg['emailsendingmethod'] == 'smtp'){
				...
				if(!$this->phpmailer_adminnotification->Send()){
					...
					$phpmailer_smtperrormessage->Body .= "\r\n".'You may want to double check your SMTP credentials in the form configuration file in order to fix this problem.';
	...
	function sendUserNotification(){
		...
		if($this->demo != 1)
		{
			...
			$this->phpmailer_usernotification->AddAddress($this->usernotification_emailaddress);
			...
			$this->phpmailer_usernotification->IsHTML(false);
				
			if($this->cfg['usernotification_format'] == 'html'){
				$this->phpmailer_usernotification->IsHTML(true);
			}
	...
	function sendAdminAPIError($service_error){
		...
		$mail_body = 'An error occured with '.$api_name.' API and the following error message was returned. ';
		...
		if($sendmail && ($error_message || $service_error['error_message'])){
			...
			$this->phpmailer_errornotification->SetFrom($this->cfg['email_address']);
