<?php
require_once('sessionpath.php');

require_once('../inc/form.config.php');
...
if($_SESSION['captcha_img_string']['165eba037494350ba50f4a326c4b0fc8ef510e99'] != $_POST['captcha_input']){
	$captcha_element_id = 'cfg-element-3-3'; // will be used in merge_post
	$captcha_elementlabel_id = 'cfg-element-3-3-label'; // will be used in element_ids_values
	$json_error_array['cfg-element-3-3']['errormessage'] = $contactform_obj->cfg['form_errormessage_captcha'];
	$error_captcha = true;
}
...
require_once('../class/class.cfgenwp.api.php');
...
if($json_error_array){
	...
	if(isset($contactform_obj->merge_post) && $contactform_obj->merge_post)
	{
		...
		// PLACE YOUR DATABASE CONNECTION CODE SNIPPET HEREw		
			...
			&& isset($cfg['campaignmonitor']['lists']) && $cfg['campaignmonitor']['lists'])
		{
			...
			if($service_requirements_status['status']){	
				...
				require_once('../api/campaignmonitor/csrest_subscribers.php');
			...
			&& isset($cfg['constantcontact']['lists']) && $cfg['constantcontact']['lists'])
		{
			...
			if($service_requirements_status['status']){
				require_once('form-validation-constantcontact.php');
			} else{
			...
			)
		{
			...
			if($service_requirements_status['status']){
				...
				require_once('../api/mailchimp/Mailchimp.php');
			...
			)
		{
			...
			if($service_requirements_status['status']){		
				...
				require_once('../api/salesforce/SforceEnterpriseClient.php');
