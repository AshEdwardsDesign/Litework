...
if(isset($cfg['constantcontact']['apikey']) && $cfg['constantcontact']['apikey']
	...
	&& isset($cfg['constantcontact']['lists']) && $cfg['constantcontact']['lists'])
	{
		...
		require_once('../api/Ctct/autoload.php');
			...
			if(isset($list_v['fields']) && $list_v['fields']){
				...
				foreach($list_v['fields'] as $fields_v){
					...
					if($fields_v['list_field_id'] == 'email'){
						$api_contact_email = $element_ids_values[$fields_v['element_id']]['element_value'];
						break;
					}
				}
			}
			...
			if($api_contact_email)
			{
				try{
					...
					if(empty($response->results)){
						...
						/*
						if(isset($list_v['addresses']) && $list_v['addresses']){
							...
							foreach($list_v['addresses'] as $address_field_k=>$address_field_v){
								...
								foreach($address_field_v['data'] as $address_data_k=>$address_data_v){
									$address->{$address_data_k} = $element_ids_values[$address_data_v['element_id']]['element_value'];
								}
					...
					if(isset($list_v['fields']) && $list_v['fields']){
						foreach($list_v['fields'] as $fields_v){
							if($fields_v['list_field_id'] != 'email'){
								$cc_contact->{$fields_v['list_field_id']} = $element_ids_values[$fields_v['element_id']]['element_value'];
							}
						}
					}
					 ...
					 */
					if(isset($list_v['groups']) && $list_v['groups'] && ($api_action == 'add' || $list_v['updateexistingcontact']) ){
