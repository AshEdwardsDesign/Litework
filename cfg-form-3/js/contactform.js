jQuery(function(){
	...
	form.on('click', 'img.cfg-captcha-refresh', function(){
		...
		form.find('img.cfg-captcha-img').prop('src','cfg-form-3/inc/captcha.php?r='+Math.random());
		
	});
	

	form.on('click', '.cfg-submit', function(){
		...
		formcontainer.find('.cfg-form-value').each(function()
		{
			...
			var element_label = element_container.find('.cfg-label-value');
			
			var element_properties = {};
			element_properties['elementlabel_id'] = element_label.closest('label').prop('id');
			element_properties['elementlabel_value'] = element_label.html();
		...
		jQuery.post('cfg-form-3/inc/form-validation.php',
					post,
					function(data){
